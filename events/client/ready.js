...
function change_status(client) {
    ...
    if (!state) {
        client.user.setActivity(`${config.status.text}`
            ...
            .replace("{songsplayed}", nFormatter(Math.ceil(client.stats.get("global", "songs") * [...client.guilds.cache.values()].length / 10), 2)), { type: config.status.type, url: config.status.url });
    } else {
